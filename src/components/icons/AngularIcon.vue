<template>
  <svg viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
    <g
      id="SVGRepo_tracerCarrier"
      stroke-linecap="round"
      stroke-linejoin="round"
    ></g>
    <g id="SVGRepo_iconCarrier">
      <path d="M16 2L3 7L5 24L16 30L27 24L29 7L16 2Z" fill="#DD0031"></path>
      <path d="M16 2V30L27 24L29 7L16 2Z" fill="#C3002F"></path>
      <path
        d="M15.9998 5.09375L7.87305 23.3638H10.9031L12.5368 19.2757H19.4348L21.0685 23.3638H24.0986L15.9998 5.09375ZM18.3736 16.7557H13.626L15.9998 11.0298L18.3736 16.7557Z"
        fill="white"
      ></path>
    </g>
  </svg>
</template>

<style scoped>
@keyframes glow {
  0% {
    -webkit-filter: drop-shadow(0px 0px 5px transparent);
    filter: drop-shadow(0px 0px 5px transparent);
  }
  50% {
    -webkit-filter: drop-shadow(0px 0px 10px rgba(227, 76, 38, 0.8));
    filter: drop-shadow(0px 0px 10px rgba(227, 76, 38, 0.8));
  }
  100% {
    -webkit-filter: drop-shadow(0px 0px 5px transparent);
    filter: drop-shadow(0px 0px 5px transparent);
  }
}

svg {
  width: 100%;
  height: 100%;
  /* animation: glow 5s infinite ease-in-out; */
  -webkit-filter: drop-shadow(0px 0px 10px rgba(221, 0, 49, 0.8));
  filter: drop-shadow(0px 0px 10px rgba(221, 0, 49, 0.8));
}
</style>
